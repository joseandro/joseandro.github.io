...
---
When we talk about proofs, one thing that easily comes to mind are the mathematical proofs of theorems that most of us have seen at some point of our lives. [Mathematical proofs](https://en.wikipedia.org/wiki/Mathematical_proof) can take many forms and follow multiple strategies. You can prove a theorem using [induction](https://en.wikipedia.org/wiki/Mathematical_induction), [contradiction](https://en.wikipedia.org/wiki/Proof_by_contradiction), [contraposition](https://en.wikipedia.org/wiki/Contraposition), etc. After the mathematical proof is written, following either strategy, anyone can follow the steps described in the proof to verify its validity. We can model these proofs using the **NP complexity class**, where the proof is the certificate (more on this in the next section).
...
In the "real world", though, people normally follow a different proof model: people try to convince others of a valid claim by interacting with them. The person who judges if a proof to a claim is true (i.e., the Verifier) normally asks multiple questions to (i.e., interrogates) the person who is providing the proof (i.e., the Prover) until she is finally convinced. They are also called **actors**. In this model, the proof is the set of questions and answers, which the Verifier analyzes before she accepts or rejects it. We will refer to this generic way of making proofs in real life as the **verifiable proof** model, where valid proofs only exist for true claims. Here is a depiction of this model:

![Verifiable proofs](img/verifiable_proofs.png#center)

This interesting, albeit natural, definition also seems more feasible (and tangible) to be implemented as an algorithm than the generic definition of mathematical proofs. This model also gives us an idea of how efficient computation through interaction can take shape: the Prover can bear the load of heavy computation while the Verifier can be tasked with simply checking if the proof is valid or not. Nevertheless, such a system is still too loosely defined and it can't be used in an untrusted environment. It would be interesting to define it more formally using computer science theory. Then, perhaps we could show how powerful and efficient it is. For example, using a formal definition, we would know if it is possible to efficiently prove to anyone that we know the answer to a game of chess! It would also be interesting to come up with a proof model that works well in untrusted environments, where any actor can cheat to gain personal advantage. Fortunately, this is an active area of research in computer science, the foundation on which zero-knowledge proofs are based on. To understand it, let's start by digging into complexity classes and the Big O notation.
...
### Turing machines
The [Turing machine (TM)](https://simple.wikipedia.org/wiki/Turing_machine) is one of the most popular models of computation, created by Sir Alan Turing himself! The theory behind a TM can get quite abstract and complex. Honestly, if you've never heard of TMs before, don't bother. You can think of it as a very simple model of computation that can implement any algorithm, like a simple computer. The deterministic version of a TM simply states that the machine will always output the same answer (e.g., `1`, `0` or `halt`) from the same inputs. The probabilistic version of a TM may (or may not) output a different answer from the same input. A deterministic TM can be turned into non-deterministic by using a [random number generator (or RNG)](https://en.wikipedia.org/wiki/Random_number_generation) to make decisions.

### Boolean circuit
A Boolean circuit (or BC) is a generalization of Boolean functions (e.g., functions that output either `0` or `1`, or `true` or `false`). Modern silicon chips used in computers started out implementing BCs, they still do. BCs are defined using a composition of logic gates, such as `AND`, `OR`, and `NOT`. The BC model of computation is mathematically simpler than the TM model.

### Interactive proof systems
We will not dig into them here since we will have a dedicated section for it later. Roughly speaking, it shapes the verifiable proof model into a model of computation. For now, just keep in mind that they are another kind of computation model. 

## Big O notation
Before we talk about complexity classes in more detail, I need to describe the [**Big O notation**](https://en.wikipedia.org/wiki/Big_O_notation). It is a mathematical notation used in computer science to describe the **run time** behavior of a program as its inputs length grows. You can think of it as the number of instructions an algorithm executes on a problem expressed as a function of its input. The Big O notation is an upper bound estimate, it classifies algorithms according to their growth rates, which may take constant \\( O(1) \\), logarithmic \\( O(\log n) \\), quasilinear \\( O(n \log n) \\), polynomial \\( O(n^c) \\), factorial \\( O(n!) \\), etc, where \\( n \\) is the binary input to the algorithm. In computer science, [**we say an algorithm is efficient if it has polynomial run time**](https://medium.com/probably-approximately-correct/polynomial-time-and-efficient-algorithms-16481666827b). This is a complex topic that I described too briefly, so if you were not previously familiar with it, I recommend you to watch [this well paced video](https://www.youtube.com/watch?v=Q_1M2JaijjQ) that will take you step by step through the whole story behind the Big O notation.
...
- \\( P \\) is the class of **decision problems** to which the answer is **yes** and can be solved in **polynomial time** (i.e., can be solved efficiently) relative to the size of the input by a deterministic Turing machine. 
- \\( NP \\) is the class of **decision problems** to which the answer is **yes** with **proofs** that can be verified in **polynomial time** (i.e., can be verified efficiently) relative to the size of the input by a deterministic Turing machine. The verification of a solution is done using the input to the problem (binary string) and the solution **certificate** (or **witness**). The witness certifies that the input is in the NP-language. As I mentioned before, you can also think of the certificate as the proof!
- \\( NPC \\) (or **NP-Complete**) is the class of the hardest problems in \\( NP \\). If you can solve an \\( NPC \\) problem you can solve any problems in \\( NP \\) (keep this fact in mind, it will be useful later). Interestingly, if you can find a solution to an **NP-Complete** problem in polynomial time, you will be solving one of the most notorious open problems in computer science and perhaps even get a [US$1 million prize](https://en.wikipedia.org/wiki/Millennium_Prize_Problems)💰🤑💰
...
In BPP, we can drive the odds of error to as low as we want by re-running the problem on a probabilistic TM with the same inputs. In this different scenario, enabled by the probabilistic model of computation, how do we **accept** or **reject**? One possibility is to, at the end of all runs, decide by simply checking the majority of the answers. However, regardless of how miniscule we drive the error down, we will still need to allow for some probability of error to happen when we **accept** or **reject**. BPP is a great option if we are have enough time to run it the model multiple times. The \\( BPP \\) class can model more realistic algorithms, give efficient approximations to computationally infeasible problems, provide better handling of errors and noise, and even analyze security in cryptography! It turns out that most problems in \\( BPP \\) can run quickly on a modern computer. The \\( BPP \\) class is another well-established theory we can incorporate to make our proofs more practical in real life! This is fantastic! 🤯 

The complexity class \\( P \\), problems solvable in polynomial time using a deterministic machine, is contained in \\( BPP \\) (\\( P \subseteq BPP \\)) because deterministic machines are just a special case of probabilistic machines. Interestingly, today is unknown if \\( BPP=P \\), since many problems that computer scientists believed to be in \\( BPP \\), but not in \\( P \\), are decreasing. 

It is worth trying to connect the dots on your own before we continue. With all the new tools in our toolbox, how would you come up with a model of computation, based on the verifiable proof model, that allows us to define proofs in an efficient and practical way? Using NP, maybe? Does that ring a **polynomial time** bell? I have introduced a lot of key ingredients that we will use to define interactive proof systems, let's dig into it.
...
The catch is that **Commit(m)** is computationally indistinguishable from **Commit(n)**! I am not going to drill down into the details here, but in essence, this is why commitment schemes are used to create computational zero-knowledge! This was a groundbreaking idea that is used nowadays in \\( ZKP \\) schemes such as **SNARKs** and **STARKs**. 
...
We have reached the end of this first sprint! This introduction gave you some tools to understand the what and why proof systems are designed the way they are. In the next post we will go over the different designs of SNARKs systems. Até breve!
