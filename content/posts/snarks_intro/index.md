...
As described by [Moore's Law](https://en.wikipedia.org/wiki/Moore%27s_law), our computing power has been increasing exponentially in the last 40 years. So, there was no clear use case as we could make the Verifier more powerful and cheaper in a matter of months. This changed when blockchains came up.

![Ethereum fees per transaction](img/eth_fees.png#center)

The picture above shows the daily average USD price users had to pay to send transactions on Ethereum in the last 3 years. In peaks of high on chain activity, it is common to see users having to pay hundreds of dollars just to get a single transaction prioritized first in a block, this is crazy! We want the whole world to use blockchains. We need to find a better solution that can scale well and accommodate new groundbreaking use cases for more individuals and companies, specially in peaks of high activity. As you can see, L1 blockchains such as Ethereum can be considered a limited, slow, and **very** expensive computer. They fit the Verifier role in SNARKs systems very well, so researchers started looking into what kind of applications they could enable with it.

Another reason why (zk)SNARK proofs work well in blockchains is they don't require interactivity. Blockchains are distributed systems. They are P2P networks consisting of many nodes that often have to replay transactions from other peers to validate blocks. If the protocol required interactivity it would not suit blockchains well, as the Prover would have to interactively prove to every node on chain that the proof is valid. As you will see next, even though scaling was the main reason for the wide adoption of SNARKs in blockchains, today it is just one among the many cool applications out there!

## zkRollups
zkRollups are normally used to scale L1 blockchains. In a zkRollup, we move the hard computation work off-chain, normally to much faster L2 chains, while the L1 stays in charge of simply verifying the work. Here is a general overview of the solution:
...
3. The Sequencer then uses SNARK system to create a proof for the correctness of the batch. This validity proof proves that the state transition happened correctly within the batch.
4. The Sequencer (or another actor) sends the validity proof and the new rollup state to smart contracts sitting on the L1. The contracts are capable of verifying such proofs and updating the state on the L1.
5. Once L1 contract verifies the SNARK, it can decide to accept or reject the new rollup state. The L2 state is normally handled by a different contract (aka rollup state) on the L1.

As you can see, a zkRollup is also a data compression mechanism as it reduces the amount of data L1s need to track due to the short size of SNARKs. Due to the fact that validity proofs are also quick to verify, smart contracts can check them cheaply. One last thing about *zkRollup* is that we don't actually use zero knowledge in it, so they use validity proofs. However, that's the name that stuck with the community and it is what most people use today.
...
1. A user wants to send an asset from a blockchain to another. To illustrate this, let's assume that the user is sending **Ether** from **Ethereum** to **Mina**.
...
## zkOracles
Blockchain smart contracts rely on Oracles to grab data that lives off-chain. A whole industry was formed around this context, with Chainlink being one of the most prominent projects in the space. Unfortunately, today smart contracts need to trust that Oracles are always giving them the right answer. Most of the existing projects sort of rely on a consensus from all Oracles responses, reporting back to smart contracts with an average response of the data their Oracles have witnessed. There are mechanisms in place to ensure Oracles aren't cheating, nonetheless you still require a whole network of Oracles with great tokenomics, security latches, and a fine tuned consensus algorithm to make it all work well. Furthermore, for every new web2 API supported, there is the need to use more Oracles and more incentives to make the whole process trustworthy. The following image gives an overview of the Oracles dynamics.

![How Oracles work](img/oracles.png#center)

zkOracles can disrupt the Oracles industry by creating ZK proofs from the whole API's response transcript and bundled with smart contract logic. For example, smart contracts (or zkApps) in Mina are executed off-chain, only the SNARK proof is submitted to the chain. We could use zkOracles to fetch the price of the Tesla stock, perform some fancy Defi computation off-chain and submit the proof to the zkApp contract that is sitting on chain. 

Oracles haven't yet enabled smart contracts to access private user information off-chain. zkOracles, however, can access private data securely in web2. Using zkOracles people could, for instance, access their bank account and prove they have more than X dollars in their name without publicly disclosing any private information. Differently from the other two applications we have seen above, zkOracles need zero knowledge to preserve user's privacy, so it uses ZK proofs. Without zero knowledge, zkORacles would not be able to properly handle private information.

![Blockchain oracles](img/blockchain_oracles.jpg#center)

zkOracles have the potential of revolutionize the entire financial industry by seamlessly connecting the web2 and web3 worlds. They could unlock the new wave of groundbreaking Defi applications. Unfortunately, no single project has yet implemented and deployed zkOracles, although protocols such as [Deco](https://www.deco.works/) have started paving the way for them. Some of the projects currently looking into zkOracles are [Mina](https://minaprotocol.com/blog/what-are-zkoracles), [Chainlink](https://blog.chain.link/deco-introduction/), and [Hyper Oracle](https://www.hyperoracle.io/). 
...
Other examples of applications using zkSNARKs to protect user's identity are: [Sismo](https://www.sismo.io/), [Humanode](https://humanode.io/), [iden3](https://iden3.io/), [Polygon ID](https://polygon.technology/polygon-id), [Sealance](https://www.sealance.io/), and [Clique](https://clique.social/).

## Privacy
ZK proofs are also used to bring privacy to blockchains. Benefited by private computation and transactions, the computation may happen on a zkVM (or off-chain) and when committed to the blockchain its transaction won't reveal any data. A zkSNARK is used to prove that private the transaction (or computation) are actually valid (i.e., correct contract execution, no double spending, properly signed, no money was lost, etc). The following projects are exploring this space: [Semaphore](https://semaphore.appliedzkp.org/), [Manta](https://manta.network/), [Tornado Cash](https://en.wikipedia.org/wiki/Tornado_Cash), [Penumbra](https://penumbra.zone/), [Aztec](https://aztec.network/), [ZCash](https://z.cash/), [Anoma](https://anoma.net/), and [Ironfish](https://ironfish.network/).

## zkML
Machine learning (ML) applications need heavy computation power and sometimes also have to deal with sensitive data. These requirements can't be supported by smart contracts on blockchains directly. Furthermore, ML models need to be trained and tested before they are ready to give predictions (or outcomes) at the inference step. Users provide an input and run the inference step on a trained model to get an outcome. Even though immense value could be unlocked from zero knowledge proofs of the training step, it is by far the heaviest and costly of all steps in ML and not really suitable to the performance of SNARKs systems today. Today *zkML* is only used to refer to proofs created for the inference step of ML models.

![Ghost in the shell brain inspection](img/ghost_shell.jpg#center)

Most zkML applications use validity proofs, though some projects are starting to explored ZK proofs as well, here are two use cases:
1. Transparency in **ML as a Service (MLaaS)** through validity proofs: MLaaS providers are becoming increasingly popular with [Google](https://cloud.google.com/ai-platform), [Azure](https://studio.azureml.net/), and [AWS](https://aws.amazon.com/machine-learning/) as a few examples. However, just like other service/platform solutions, MLaaS are opaque black-boxes to clients. Users send model inputs to APIs that capture and run inference in models behind closed doors. How can users trust they are actually getting predictions from models they have paid for? MLaaS providers can enable transparency by attaching validity proofs to API responses! This way users can quickly verify that the results they are getting are actually coming from the inputs they provided to the ML models they have paid for. 
2. ZK proofs for **privacy preserving ML**: zkSNARKs can be used to bring privacy to critical ML applications such as in healthcare. The ML model would create a ZK proof that the private user inputs (CT scan, blood exams, X-rays, etc) correspond to a given diagnosis prediction. These proofs could be verified by third parties, such as insurance companies without revealing any private information given as part of the input. Consider the [case](https://eprint.iacr.org/2020/584.pdf) for disease detection:
...
## What is next?
Blockchain interest and industry investments propelled the development of SNARKs systems in many ways. These systems were not so performant 10 years ago as they are today, they used to take a very long time just to even generate proofs for small statements. The key breakthrough in newer SNARK systems is happening in the development of much faster Provers. Newer systems are capable of generating proofs in linear (or quasilinear) time, mostly possible thanks to many interesting clever ideas coming from algebra. In the next post I will start covering these cool ideas as I introduce the design of modern SNARK systems.
